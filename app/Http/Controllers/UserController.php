...
public function updateUser(Request $request, $id)
{
    try {
        ...
        if ($inputPassword) {
         ...
         /* $userRole = user_role::where('user_id', $user->id)->firstOrFail();
        if ($userRole->user_id == $request->input('cmbuserRole')) {*/


       /* if( $user->name == $request->input('txtname')&&
        $user->email ==$request->input('txtEmail') &&
        $user->user_id == $request->get('cmbuEmployee') &&
        $user->user_type == ($request->input('cmbuserTypeRole') == 0 ? 'Guest' : 'Employee')
        ...
        if ($user) {
            ...
            // Uncomment the code below if you have a separate table for user roles
        ...
        if (Hash::check($inputPassword, $user->password)) {
            ...
            if ($users) {
                $users->name = $request->input('txtname');
                $users->email = $request->input('txtEmail');
                $user_id = $request->input('cmbuEmployee');
                $users->user_id = ($user_id !== 'null') ? $user_id : null;
                $user_type = $request->input('cmbuserTypeRole');
                $users->user_type = ($user_type == 0) ? 'Guest' : 'Employee';
                $users->save();

                // Uncomment the code below if you have a separate table for user roles

                $userRoleId = $request->input('cmbuserRole');
                $userRole = user_role::where('user_id', $users->id)->firstOrFail();
            ...
            return response()->json(['error' => 'User password not match']);
        }
